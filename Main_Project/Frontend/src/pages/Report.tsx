...
type ValidationBadgeResult = Pick<ValidationResult, "status" | "errors">;
...
const reportApi = {
  get: (runId: string) => sessionApi.getReport(runId) as Promise<ReportResponse>,
};
...
function getVisualizationSrc(url: string | null | undefined) {
  if (!url) return null;
  ...
  if (url.startsWith("/storage/")) return `/api/ai${url}`;
  if (url.startsWith("storage/")) return `/api/ai/${url}`;
  return url;
}

function getVisualizationRequestPath(url: string | null | undefined) {
  if (!url) return null;
  if (/^(https?:)?\/\//i.test(url) || url.startsWith("data:") || url.startsWith("blob:")) {
    return null;
  }
  if (url.startsWith("/api/")) return url.replace(/^\/api/, "");
  if (url.startsWith("/ai/")) return url;
  if (url.startsWith("/storage/")) return `/ai${url}`;
  if (url.startsWith("storage/")) return `/ai/${url}`;
  return null;
}

async function fetchVisualizationHtml(path: string) {
...
function VisualizationEmbed({ url, runId }: { url: string | null | undefined; runId: string }) {
  const src = getVisualizationSrc(url);
  const requestPath = getVisualizationRequestPath(url);
  const {
    data: dashboardHtml,
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["visualization-html", runId, requestPath],
    queryFn: () => fetchVisualizationHtml(requestPath!),
  ...
  return (
    <div className="card p-5">
      ...
      <div className="rounded-xl overflow-hidden border border-border">
        {isLoading ? (
          <div className="h-[600px] flex items-center justify-center bg-muted/40 text-sm text-muted-foreground">
            Loading dashboard...
          </div>
        ) : (
          <iframe
            key={`${runId}-${dashboardHtml ? "html" : "src"}`}
            src={dashboardHtml ? undefined : src}
            srcDoc={dashboardHtml || undefined}
        ...
        {isError && (
          <div className="border-t border-border bg-muted/30 px-4 py-3 text-xs text-muted-foreground">
            Dashboard preview could not be embedded.{" "}
...
function ValidationBadge({ result }: { result?: ValidationBadgeResult }) {
...
export default function Report() {
  ...
  const reportContent = data.report || data.final_report || "";
  const displayRunId = data.run_id || runId || "";
  const handleBackToChat = () => {
    navigate(currentSessionId ? `/agent?session=${encodeURIComponent(currentSessionId)}` : "/agent");
  };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="animate-slide-up">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 font-mono">{displayRunId}</p>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            {data.visualization_url && (
              <div className="animate-slide-up" style={{ animationDelay: "250ms", animationFillMode: "both" }}>
                <VisualizationEmbed url={data.visualization_url} runId={displayRunId} />
              </div>
            )}
          ...
          <div className="space-y-5">
                  ...
                  {Object.entries(data.user_profile).map(([key, val]) => (
                    <div key={key} className="flex justify-between items-start gap-2">
                      ...
                      <span className="text-xs text-foreground font-semibold text-right">
                        {Array.isArray(val) ? val.slice(0, 5).map(String).join(", ") : String(val)}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="animate-slide-up" style={{ animationDelay: "150ms", animationFillMode: "both" }}>
              <ValidationBadge result={data.validation_result as ValidationBadgeResult | undefined} />
            </div>
